...
class ApiMovieDb extends MovieDB {
  ...
  Future<List<Movie>> getMovieByGanre(int selectedGanre) async {
    try {
      final url =
          '${MovieDB.baseUrl}/discover/movie?with_genres=$selectedGanre&${MovieDB.apiKey}';
      final response = await _dio.get(url);
      var movies = response.data['results'] as List;
      List<Movie> movieList = movies.map((m) => Movie.fromJson(m)).toList();
  ...
  Future<MovieDetail> getMovieDetail(int movieId) async {
    try {
      final response =
          await _dio.get('${MovieDB.baseUrl}/movie/$movieId?${MovieDB.apiKey}');
  ...
  Future<List<Screenshot>> getMovieImage(int movieId) async {
    try {
      final response = await _dio
          .get('${MovieDB.baseUrl}/movie/$movieId/images?${MovieDB.apiKey}');
      var screenshots = response.data['backdrops'] as List;
      List<Screenshot> screenshotList =
          screenshots.map((m) => Screenshot.fromJson(m)).toList();
  ...
  Future<List<Cast>> getCastList(int movieId) async {
    try {
      final response = await _dio
          .get('${MovieDB.baseUrl}/movie/$movieId/credits?${MovieDB.apiKey}');
      var list = response.data['cast'] as List;
      List<Cast> castList = list.map((c) => Cast.fromJson(c)).toList();

      return castList;
    } catch (error) {
      throw Exception('Exception with error cast: $error');
    }
  }
  ...
  Future<Person> getPerson(int personId) async {
    try {
      final response = await _dio
          .get('${MovieDB.baseUrl}/person/$personId?${MovieDB.apiKey}');
  ...
  Future<List<Movie>> getSearchMovieList(String query) async {
    try {
      final response = await _dio.get(
          '${MovieDB.baseUrl}/search/movie?${MovieDB.apiKey}&query=$query');
      var movies = response.data['results'] as List;
      List<Movie> movieList = movies.map((m) => Movie.fromJson(m)).toList();
      print(movieList);
      return movieList;
